module SetupProgress
  ...
  included do
    helper_method :setup_step, :setup_total_steps, :setup_incomplete?
  end
  ...
  private def setup_step
    return 1 unless EmulatorProfile.where(user_selected: true).exists?
    return 2 unless Game.exists?
