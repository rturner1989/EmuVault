...
VAPID keys are generated automatically during `install.sh`. If you need to regenerate them (e.g. after cloning onto a new machine with an existing `.env`):
...
All configuration is in `.env` (generated by `install.sh`). Key variables:
...
| `VAPID_PUBLIC_KEY` | Web push public key (auto-generated by `install.sh`) |
| `VAPID_PRIVATE_KEY` | Web push private key (auto-generated by `install.sh`) |
...
The `/activity` page shows a full history of all uploads and downloads, including timestamp, game, device type, and IP address.
...
Available at `/sidekiq`, protected by HTTP Basic Auth using your `ADMIN_EMAIL` / `ADMIN_PASSWORD`.
